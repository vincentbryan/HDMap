...
void filter_callback(const sensor_msgs::PointCloud2::ConstPtr &pcl_msg, const nox_msgs::Location::ConstPtr &imu_msg){
//  ROS_INFO_STREAM("update pointcloud");
    ...
    double y = imu_msg->y;
    double z = imu_msg->z;
    double imu_yaw = (( imu_msg->yaw) + 90.0 ) * M_PI / 180.0 ;
    double imu_pitch = imu_msg->pitch  * M_PI / 180.0 ;
    double imu_roll = imu_msg->roll  * M_PI / 180.0 ;
//    ROS_INFO_STREAM("gps: " << imu_msg->x << " " << imu_msg->y << " " << imu_msg->z << " "  );
//    ROS_INFO_STREAM("lidar: " << velo2imu_XYZ[0] << " " << velo2imu_XYZ[1] << " " << velo2imu_XYZ[2] << " "  );
    tf::Transform gps_pose;
    gps_pose.setOrigin( tf::Vector3(x,y,z) );
    tf::Quaternion q_gps;
    q_gps.setRPY(imu_roll, imu_pitch, imu_yaw);
    gps_pose.setRotation(q_gps);
...
//    sor.filter(*velodyne_global_save);
    ...
    point_pub.header.frame_id="/world";
    ...
    br.sendTransform(tf::StampedTransform(tf_pub, ros::Time::now(), "velodyne", "/world"));
}
...
int main(int argc, char **argv){
    ...
    message_filters::Subscriber<nox_msgs::Location> imu_sub(nh_,"/Localization",200);

    typedef message_filters::sync_policies::ApproximateTime< sensor_msgs::PointCloud2, nox_msgs::Location> MySyncPolicy;
